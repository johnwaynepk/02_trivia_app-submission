...
def retrieve_categories():
    ...
    for item in categories:
      all_categories[item.id] = item.type
...
def create_app(test_config=None):
  ...
  def add_questions():
    ...
    try:
      if search:
        ...
        return jsonify({
            'success': True,
            'added question': new_question.question
          })
  ...
  def get_quiz():
    ...
    if quizCategory['id'] == 0:
      ...
      questionList = Question.query.filter(Question.category == quizCategory['id']).order_by(Question.id).all()
